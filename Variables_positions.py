...
#--------- Position du lanceur et des caméras ---------

position_launcher = [3000,3000]  # Position du lanceur par rapport à la base du terrain (en mm) : (abscisse, ordonnée) 
position_cameras = [3000,3000]   # Position de l'installation caméra par rapport à la base du terrain (en mm) : (abscisse, ordonnée)

abscissa_difference = position_cameras[0] - position_launcher[0]
ordinate_difference = position_cameras[1] - position_launcher[1]
...
def circle_difficulty_on_court(depth_player,width_player,position_launcher,radius_difficulty):
    """
    But : Renvoie True si une partie du cerle de difficulté est dans le terrain, sinon le point le plus proche dans le terrain.
    """
    depth = False
    width = 0
...
#--------- Calculs de la profondeur/largeur en fonction de l'installation des caméras ---------

def depth_cameras(angle_left,angle_right,baseline):
    """
    Calcule la profondeur par rapport aux caméras d'un joueur sur le terrain en utilisant les angles de caméra gauche et droite, ainsi que la distance de base entre les caméras.
    """
...
def width_cameras(angle_left,angle_right,baseline):
    """
    Calcule la largeur par rapport aux caméras du joueur (distance horizontale) à partir des angles de la caméra gauche et droite.
    """
    tal = np.tan(angle_left)
    bl = baseline/2
    depth = depth_cameras(angle_left,angle_right,baseline)
    
    return -((depth/tal)+bl)

#--------- Calculs de la profondeur en fonction du lanceur ---------

def depth_launcher(angle_left,angle_right,baseline,ordinate_difference):
    """
    Calcule la profondeur par rapport au lanceur d'un joueur sur le terrain en utilisant les angles de caméra gauche et droite, ainsi que la distance de base entre les caméras.
    """
    depth_cam = depth_cameras(angle_left,angle_right,baseline)

    return depth_cam + ordinate_difference

def width_launcher(angle_left,angle_right,baseline,abscissa_difference):
    """
    Calcule la largeur par rapport au lanceur du joueur (distance horizontale) à partir des angles de la caméra gauche et droite.
    """
    width_l = width_cameras(angle_left,angle_right,baseline)
    
    return width_l + abscissa_difference
...
def position_on_court(height_img,width_img,depth,width):
    """
    But : Convertir une position réelle (mm) en pixels sur l’image de représentation.
    """
    scale = dimension_scale()  # Calcul du facteur d'échelle pour conversion mm -> pixels
    depth = int(depth/scale)   # Conversion de la profondeur en pixels
    width = int(width/scale)   # Conversion de la largeur en pixels

    y = int(height_img - depth)  # Ajustement de la profondeur pour correspondre au repère de l'image

    # Calcul de la position horizontale du joueur en fonction du centre du terrain
    if width < 0:
        x = int((width_img/2) - abs(width))  # Joueur à gauche du centre
    else:
        x = int((width_img/2) + width)  # Joueur à droite du centre
    return x, y

def position_difficulty_on_court(frame,position_launcher,depth,width,level,radius,real_condition_launcher):
    """
...
def player_variable(frame,baseline,position_left,position_right,vision_field_left,vision_field_right):
    ...
    depth_player = depth_launcher(total_angle_left,total_angle_right,baseline,ordinate_difference)
    width_player = width_launcher(total_angle_left,total_angle_right,baseline,abscissa_difference) 
...
def difficulty_variable(frame,depth_player,width_player,level_difficulty,radius_difficulty,real_condition_launcher):
    ...
    position_player_court = position_on_court(height_frame,width_frame,depth_player,width_player)

    position_player_court_base = position_court_base(frame,position_player_court)
    position_difficulty_court = position_difficulty_on_court(frame,position_launcher,position_player_court[1],position_player_court[0],level_difficulty,radius_difficulty,real_condition_launcher)
...
def permanent_variable(frame,position):
    ...
    position_permanent_court = position_on_court(height_frame,width_frame,position[0],position[1])
